...
def test_python_repl_execute_arithmetic():
    result = python_repl.invoke({"code": "print(6 * 7)"})
    assert "42" in result
...
def test_web_search_respects_disable_flag():
    """Test that web_search respects the disable flag."""
    import app.agent.tools as tools_module
    import importlib
    from app.config import settings
    
    with patch.object(settings, 'enable_web_search', False):
        # Перезагружаем модуль
        importlib.reload(tools_module)
        
        result = tools_module.web_search.invoke({"query": "anything"})
        assert "disabled" in result.lower() or "отключ" in result.lower()