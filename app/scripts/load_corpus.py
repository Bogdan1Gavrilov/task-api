...
Делает 4 вещи подряд: тянет HTML страницы по списку URL,
читает локальные `*.md` из `data/local`, чистит HTML от script/nav/footer
и режет результаты на чанки фиксированного размера. Финальный 
результат - `data/corpus_chunks.jsonl`, который едет дальше в `index_corpus.py`
"""
...
#Три самых "базовых" вопроса Classic ML
SEED_URLS = [
...
def clean_html(html: str) -> str:
    """Выкинуть scripts/style/nav/header/footer и вернуть чистый текст страницы"""
...
def chunk_documents(docs: list[Document]) -> list[Document]:
    """Нарезать документы на куски CHUNK_SIZE симлолов с оверлепом"""
