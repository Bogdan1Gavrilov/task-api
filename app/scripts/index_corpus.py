"""Индексация чанков из `data/corpus_chunks.jsonl` d Qdrant.

Пересоздаём коллекцию (чистый старт каждый раз),
считаем эмбеддинги через `multilingual-e5-small` и сразу проверяем,
что ретривер ходит: три sanity запроса (EN / RU / meta вопрос).
Все параметры - из `app.config.settings`, никаких логических констант.
"""
...
def load_chunks(path: Path) -> list[Document]:
    """Прочитать jsonl и собрать коллекцию Document с метаданными"""
    docs = []
...
def main() -> None:
    """Пересоздать коллекцию Qdrant, проиндексровать чанки прогнать sanity-check"""
    client = QdrantClient(url=settings.qdrant_url)

    #Пересоздаём коллекцию: чистый старт упрощает отладку
    #В проде вместо recreate_collection делают upsert по batch
    ...
    #Sanity: один запрос на английском, один на русском и мета вопрос
    for query in [
