...
вместе с ретривером (он отдкльно нужен для оценки).
`format_docs_with_sources` склеивает top-k чанков в нумерованный
...
the service`s internal "about" page.

Rules:
- Use ONLY the provided context. If the answer is not in the context, say no honestly.
...
def format_docs_with_sources(docs: list[Document]) -> str:
    """Склеить top-k чанков в нумерованный context блок для промпта в LLM"""
    lines = []
