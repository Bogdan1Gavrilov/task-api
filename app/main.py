...
# LaTex разделители для Gradio Chatbot. LLM ответы про Ridge, Lasso,
# метрики содержат формулы $$..$$ / \[..\] / $..$ - без этого заголовка они отрисуются
...
class DisableProxyBufferingMiddleware(BaseHTTPMiddleware):
    ...
    Gradio`s SSE-streaming endpoints.
    Без этого live-обновления статуса приходят одним пакетом в конце.
    """
...
def _format_timings(retrieval_ms: float, llm_ms: float | None, llm_error: str | None) -> str:
    ...
    if llm_ms is not None:
        ...
        lines.append(f"**LLM call**: X {llm_error}")
    return "\n".join(lines)
...
def respond(message: str, history: list):
    ...
    # Yield #1L sources уже на экране, LLM еще не начал писать
    ...
    try:
        ...
        history[-1]["content"] = (
            f"!!!LLM провайдер сейчас недоступен ({type(exc).__name__}). "
...
def respond_agent(message: str, history: list, thread_id_state: str):
    ...
    try:
        ...
        history = history + [
            ...
            {"role": "assistant", "content": f"Запрос отклонен guardrails:{e}"},
        ]
        yield history, "", "_Отклонен guardrails_", "_-_", "_-_", "Guardrail отклонил запрос", thread_id_state
        return
    ...
    for stream_mode, payload in _agent_graph.stream(
        ...
        if stream_mode == "updates":
            ...
            # печатаем только функциональный ответ агента, а не reasoning промежутки tool вызовов
            if chunk_text and meta.get("langgraph_node") == "agent" and not getattr(msg_chunk,
            "tool_call_chunks", None):
                accumulated += chunk_text
                history[-1]["content"] = accumulated
                yield history, "", "_..._", "_-_", "_-_", f"LLM пишет потоковый ответ... {len(accumulated)} симв.", thread_id
    ...
    trace_md = "\n\n".join(trace_md_lines) if trace_steps else "_Без tool-вызовов"
    timings = (
        f"### Тайминги\n\n"
        f"- **Total:** {total_ms} ms\n"
...
def _route_respond(message, history, mode, thread_id_state):
    if mode == "Агент (/agent)":
        ...
        # Старый /chat хэндлер - добавляем пустой trace и нейтральный статус
        for out in respond(message, history):
            yield (*out, "_(режим Быстрый - trace не используется)_", "Готово", thread_id_state)


# CSS делает 3 вещи: 1) распахивает контейнер на всю ширину,
...
with gr.Blocks(title="ML Q&A", fill_height=True) as demo:
    ...
    # Всё это ДОЛЖНО быть внутри блока with
